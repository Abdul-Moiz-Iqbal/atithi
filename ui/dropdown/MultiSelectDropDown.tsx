...
const MultiSelectDropdown = ({ options, placeholder, onOptionsChange, className }) => {
  ...
  return (
    <div className={` relative flex flex-col mt-4`} ref={dropdownRef}>
      ...
      >
        <div
          className="flex-grow max-w-60 pl-2 overflow-hidden whitespace-nowrap text-ellipsis"
      ...
      {showDropdown && (
        <div
          className="absolute z-10 border-2  border-main-red overflow-y-auto max-h-[200px] w-full bg-white custom-scrollbar"
          style={{ top: "100%", left: 0 }}
        >
          {options.map((option) => (
            <div
              key={option.value}
              className={`p-2 text-slate-500 text-sm hover:text-main-red hover:bg-gradient-to-r hover:from-[rgba(255,0,0,0.3)] hover:to-[rgba(233,233,233,0)] cursor-pointer flex items-center ${
